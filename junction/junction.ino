// ═══════════════════════════════════════════════════
//  ESP32 #3 — JUNCTION CONTROLLER
//  MAC: 78:1C:3C:B7:D8:84
//
//  Runs 4-phase FSM traffic cycle
//  Handles emergency preemption via ESP-NOW
//  Handles manual override from dashboard
//
//  PIN MAP:
...
void resumeNormal() {
  ...
  goTo(NS_GREEN);
  applyNSGreen();
  Serial.println("[FSM] Normal resumed");
}
...
void loop() {
  ...
  if (eFlag && (now - eTs > TIMEOUT_MS)) {
    eFlag = false;
    Serial.println("[TIMEOUT] cleared");
  }

  switch(state) {
    case NS_GREEN:
      if (eFlag) { savedState=NS_GREEN; goTo(SAFE_TRANSITION); applyNSYellow(); }
      else if (now-stateTimer > GREEN_MS) { goTo(NS_YELLOW); applyNSYellow(); }
      break;
    ...
    case EW_GREEN:
      if (eFlag) { savedState=EW_GREEN; goTo(SAFE_TRANSITION); applyEWYellow(); }
      else if (now-stateTimer > GREEN_MS) { goTo(EW_YELLOW); applyEWYellow(); }
      break;
    ...
    case ALLRED_BUFFER:
      if (now-stateTimer > ALLRED_MS) {
        if (resuming) {
          ...
          Serial.println("[FSM] Resumed");
        } else {
          goTo(EMERGENCY_HOLD);
          applyEmergency(eDir);
          Serial.printf("[FSM] EMERGENCY dir=%d\n", eDir);
        }
      }
      break;
